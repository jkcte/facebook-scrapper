...
# Usage:
#   pip install chardet
#   py har_extract_fb_serp.py "path\to\file.har" --out results.csv [--filter "keyword"]
#
...
def strip_fb_prefix(s: str) -> str:
    if not s: return s
    s = s.lstrip()
    if s.startswith("for (;;);"): return s[10:].lstrip()
    return s
...
# ---- Generic message finder when no serpResponse ----
...
def main():
    ...
    ap.add_argument("--filter", type=str, default=None, help="Keep only messages containing this (case-insensitive).")
    ap.add_argument("--out", type=str, default="har_extracted_texts.csv", help="Output CSV filename")
    args = ap.parse_args()
